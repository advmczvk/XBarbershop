...
exports.getServiceById = (req, res, next) => {
    ...
    ServiceRepository.getServiceById(serId)
        .then(ser => {
            if(!ser) {
                res.status(404).json({
                    message: 'Service with id: '+serId+' not found'
                })
            } else {
...
exports.createService = (req, res, next) => {
    ServiceRepository.createService(req.body)
        .then(newObj => {
...
exports.updateService = (req, res, next) => {
    ...
    ServiceRepository.updateService(serId, req.body)
        .then(result => {
           res.status(200).json({message: 'Service updated!', ser: result});
        })
        .catch(err => {
            if (!err.statusCode) {
                err.statusCode = 500;
...
exports.deleteService = (req, res, next) => {
    ...
    ServiceRepository.deleteService(serId)
        .then(result => {
            res.status(200).json({message: 'Removed employee', ser: result});
        })
        .catch(err => {
            if (!err.statusCode) {
                err.statusCode = 500;
